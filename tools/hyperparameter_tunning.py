...
import pandas as pd
import numpy as np
from transformers import CompleteTransformer
...
class RandomSearch_KF:

    def __init__(self, estimator, param_grid, n_folds, n_iter, random_state):
        """
        ...
        - random_state: int
            Seed for reproducibility in shuffling parameters and splitting the data into training and test.
        ...
        self.results_dict = {}
        self.best_r2_test = 0.0001 # Initialize best R² as very low
        self.best_r2_train = 0.0001 # Initialize worst R² as very low
        self.best_params = None
        self.worst_r2_test = 0.9999  # Initialize worst R² as very high
        self.worst_r2_train = 0.9999 # Initialize worst R² as very high
    ...
    def fit(self, data, var_x, var_y):
        ...
        print(f'Running {self.n_iter} combinations randomly...\n')
        ...
        for index, combination in enumerate(param_combinations[:self.n_iter]):
            ...
            # Print status
            sys.stdout.write(f"\rIteration {index+1}/{self.n_iter} - Avg test R²: {r2_comb_test_avg:.4f} | Avg train R²: {r2_comb_train_avg:.4f} - Time: {iter_elapsed_time:.2f} min ")
            sys.stdout.flush()
        ...
        print(f"\n\nTotal time for {self.n_iter} iterations: {elapsed_time:.2f} minutes")
